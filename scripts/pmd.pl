...
sub GetPmdBugObject() {
    ...
    ###################
    $bugObject->setBugLocation(1,$class,$adjustedFilePath,$beginLine,$endLine,$beginColumn,$endColumn,$loc_msg,'true','true');
        $bugObject->setBugMessage($message);
        $bugObject->setBugBuildId($build_id);
        $bugObject->setClassAttribs($class,$adjustedFilePath,$beginLine,$endLine,""); 
