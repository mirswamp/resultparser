...
sub parseViolations
{
    ...
    my $bugObject = getCheckstyleBugObject( $elem,$xmlWriterObj->getBugId(), $bug_xpath );
...
sub getCheckstyleBugObject()
{
    ...
    ###################
    $bugObject->setBugLocation(1,"",$adjustedFilePath,$beginLine,$endLine,$beginColumn,0,"",'true','true');
    ...
    $bugObject->setBugPath($bug_xpath."[".$file_Id."]"."/error[".$bugId."]");
